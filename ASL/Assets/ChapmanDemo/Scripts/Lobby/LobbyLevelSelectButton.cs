...
public class LobbyLevelSelectButton : MonoBehaviour
{
    ...
    void buttonClicked(GameObject myButton)
    {
        //Debug.Log("Click");
        ...
        flr[0] = value + 1f;
        if (myButton.name == "ArrowRight" && this.gameObject.name == "ArrowRight")
        {
        ...
        else if (myButton.name == "ArrowLeft" && this.gameObject.name == "ArrowLeft")
        {
            LobbyPrefab.GetComponent<ASL.ASLObject>().SendAndSetClaim(() =>
            {
                LobbyPrefab.GetComponent<ASL.ASLObject>().SendFloatArray(flr);
    ...
    public void buttonClickedWithVR()
    {
